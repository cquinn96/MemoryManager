...
@implementation EditReminderVC

@synthesize myDate, tempRepeat, dateFormatter, detailItem, tempRepeatBeforeSelection, tempSound, tempSoundBeforeSelection, soundFilePath, pictureFilePath;
...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        self.title = NSLocalizedString(@"Edit Reminder", @"Edit Reminder");
...
- (void)viewDidLoad
{
    ...
    titleTextField.text =  [[detailItem valueForKey:@"reminderName"] description];
    ...
    tempRepeatBeforeSelection = [self.detailItem valueForKey:@"reminderRepeatInterval"];
    tempSoundBeforeSelection = [self.detailItem valueForKey:@"reminderSound"];
...
- (void)insertNewReminder:(id)sender
{
    ...
    if(titleTextField.text.length == 0) {
        ...
        _reminder = detailItem;
        _reminder.reminderName = titleTextField.text;
        if (_audioVC) {
            _reminder.reminderAudio = _audioVC.soundFilePath;
        }
        else {
            _reminder.reminderAudio = detailItem.reminderAudio;
        }
        if (_pictureVC) {
            _reminder.reminderPicture = _pictureVC.pictureFilePath;
        }
        else {
            _reminder.reminderPicture = detailItem.reminderPicture;
        }
        myDate = [self setSecondsToZero:myDate];
        _reminder.reminderDate = myDate;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row==1) {
        ...
        if(_audioVC)
            ...
            cell.detailTextLabel.text = soundFileExists ? @"Yes": @"No";
            if(soundFileExists){
                [_audioVC.audioSwitch setOn:YES animated:NO];
            }
        }
    }
    if(indexPath.row==2) {
        ...
        if(_pictureVC)
            ...
            cell.detailTextLabel.text = pictureFileExists ? @"Yes": @"No";
        }
    }
    if(indexPath.row==3){
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:CellIdentifier];
