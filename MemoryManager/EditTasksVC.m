...
@interface EditTasksVC ()

@end

@implementation EditTasksVC

@synthesize fetchedResultsController = __fetchedResultsController;
@synthesize managedObjectContext = __managedObjectContext;
@synthesize taskArray, tempTask, detailItem, taskSet, tasksToBeShownArray;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        self.title = NSLocalizedString(@"Edit Tasks", @"Edit Tasks");
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    self.tasksToBeShownArray = [[[NSArray alloc] initWithArray:[self.taskSet allObjects]] sortedArrayUsingDescriptors:[NSArray arrayWithObject:sortDescriptor]];

    taskArray = [NSMutableArray new];
    
    for (int i=0; i<[tasksToBeShownArray count]; i++) {
        [taskArray addObject:[[tasksToBeShownArray objectAtIndex:i] taskName]];
    }
...
- (void)configureCell:(UITableViewCell*)cell forIndexPath:(NSIndexPath*)ip {
    NSString * s = [self.taskArray objectAtIndex:ip.row];
